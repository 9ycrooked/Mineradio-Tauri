...
- **`tauri.conf.json` release CSP**：`csp` 已收紧为 `default/style 'self'`，图片/媒体只允许 `self`、`data:`/`blob:`（图片/媒体需要时）和 `http://127.0.0.1:*` sidecar；`script-src`/`connect-src` 额外只允许 baseline AI depth 的 `https://cdn.jsdelivr.net/npm/@xenova/transformers@2.17.2` 固定版本路径、ONNX WASM 所需的 `'wasm-unsafe-eval'`、`Xenova/depth-anything-small-hf` repo/cache 路径和当前模型 CDN 前缀。`devCsp` 只为 Vite/HMR 放宽 `127.0.0.1:5173`、`unsafe-eval` 和 inline style；不要把 QQ/网易云/Open-Meteo 等外部域直接加进 release CSP，远程封面和音频必须继续走 sidecar `image-proxy` / `audio-proxy`。
