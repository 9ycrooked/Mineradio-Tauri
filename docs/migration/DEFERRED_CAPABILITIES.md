...
| QQ 独立 sidecar | done | 已决策继续使用一个 Bun API sidecar，并在其内部通过统一 `ProviderAdapter` 承载 QQ provider；当前复杂度未达到拆独立进程的必要性 | 无；若后续稳定性证据要求拆分，再新增单独架构计划 | 发布前不拆独立 QQ sidecar；QQ 核心能力仍由 capability gate 验收 |
...
- `QQ 独立 sidecar` 已决策为继续单 Bun sidecar；它只是进程边界决策，不是 QQ provider 功能延期，QQ search/songUrl/lyric/playlistDetail/loginStatus/logout parity 仍必须由 capability gate 验收。
